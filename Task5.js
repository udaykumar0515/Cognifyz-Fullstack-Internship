...
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'Task5.html'));
});

app.post('/submit', (req, res) => {
  const {
    fname, lname, email, phone, address, country, otherCountry,
    education, educationStatus, day, month, year, skills, weeks, domains, password, confirmPassword
  } = req.body;
...
function storeData(data, callback) {
  fs.appendFile('D:\\uday\\projects\\nodejs\\submissions.txt', data + '\n', callback);
}
